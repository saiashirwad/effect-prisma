...
import { toCamelCase, toSnakeCase, mapPrismaTypeToEffectSchema } from "./utils.js";
...
export function generateModelSchema(
  ...
  const fieldSchemas = scalarFields
    .map((field) => {
      const schemaType = mapPrismaTypeToEffectSchema(field);
  ...
  const hasDecimalFields = scalarFields.some((f) => f.type === "Decimal");
  ...
  const decimalImport = hasDecimalFields
